...
void DialWidget::setMinMax(double minV, double maxV)
{
    if (maxV <= minV) return;
    m_min = minV;
    m_max = maxV;
...
void DialWidget::setValue(double v)
{
    if (v < m_min) v = m_min;
    if (v > m_max) v = m_max;
    if (m_value == v) return;
...
static inline QPointF polar(const QPointF& c, qreal r, qreal deg)
{
    ...
    return { c.x() + r * qCos(a), c.y() + r * qSin(a) };
}

static void drawArrow(QPainter& p, const QPointF& tail, const QPointF& tip, qreal width)
{
    QPen shaft(p.pen());
    ...
    const qreal head = std::max<qreal>(width * 2.6, 14.0);
...
void DialWidget::paintEvent(QPaintEvent*)
{
    ...
    const qreal topMargin = std::max<qreal>(72.0, h * 0.16);
    const qreal availH = std::max<qreal>(1.0, h - topMargin);

    const qreal D = std::min<double>(w * 0.86, 2.0 * availH * 0.92);
    const qreal thick = std::max<qreal>(6.0, D * 0.085);

    const QRectF arcRect((w - D) / 2.0, topMargin + (availH * 0.95 - D / 2.0), D, D);
    const QRectF ringRect = arcRect.adjusted(thick/2.0, thick/2.0, -thick/2.0, -thick/2.0);

    const QPointF c = arcRect.center();
    const qreal rOuter = ringRect.width() / 2.0;
    const qreal rInner = rOuter - thick * 0.55;
    ...
    for (int i = 0; i < steps; ++i) {
        const qreal t0 = qreal(i) / steps;
        const qreal t1 = qreal(i + 1) / steps;
    ...
    const qreal labelY = ringRect.top() - thick * 0.55;
    const QColor textClr(25,25,25);
    const QColor arrowClr(25,25,25);
    const qreal arrowW = std::max<qreal>(2.0, thick * 0.08);
    const qreal pad = std::max<qreal>(8.0, thick * 0.35);
    ...
    const qreal ang = 180.0 - 180.0 * qBound(0.0, t, 1.0);
